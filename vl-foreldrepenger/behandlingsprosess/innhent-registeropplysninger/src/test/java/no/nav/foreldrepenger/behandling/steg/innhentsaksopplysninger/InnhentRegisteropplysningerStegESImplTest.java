...
public class InnhentRegisteropplysningerStegESImplTest {
    ...
    private ArrayList<BehandlingSteg> lagInnhentRegisterOpplysningerSteg() {
        ...
        OpplysningsPeriodeTjeneste opplysningsPeriodeTjeneste = new OpplysningsPeriodeTjenesteImpl(skjæringstidspunktTjeneste, Period.of(1, 0, 0), Period.of(0, 4, 0), Period.of(0, 6, 0), Period.of(1, 0, 0));
        ...
        behandlingStegs.add(new InnhentRegisteropplysningerStegESImpl(repositoryProvider, registerdataInnhenter));
    ...
    private InntektsInformasjon opprettTestdataForInntekt() {
        ...
        /*
         * månedsinntekter.add(new Månedsinntekt.Builder()
         * .medArbeidsgiver(UTBETALER2)
         * .medBeløp(UTBETALER2_MND4)
         * .medMåned(TOM)
         * .medYtelse(false)
         * .build());
         *
         ...
         * månedsinntekter.add(new Månedsinntekt.Builder()
         * .medArbeidsgiver(UTBETALER2)
         * .medBeløp(UTBETALER2_MND3)
         * .medMåned(FOM.plusMonths(2))
         * .medYtelse(false)
         * .build());
         */
...
/*
 * fra: RegisterdataInnhenterTest
 *
 * private static final String UTBETALER1 = "Sykepenger";
 * private static final BigDecimal UTBETALER1_MND1 = new BigDecimal(500);
 * private static final BigDecimal UTBETALER1_MND2 = new BigDecimal(200);
 * private static final String UTBETALER2 = "123456789";
 * private static final BigDecimal UTBETALER2_MND2 = new BigDecimal(400);
 * private static final BigDecimal UTBETALER2_MND3 = new BigDecimal(600);
 * private static final BigDecimal UTBETALER2_MND4 = new BigDecimal(620);
 * private static final YearMonth FOM = YearMonth.now().minusMonths(3);
 * private static final YearMonth TOM = YearMonth.now();
 * private static final String NAVN = "Hans Frisk-Igjen";
 *
 * @Test
 * public void skal_aggregere_inntekt_pr_utbetaler() {
 * // Arrange
 * when(inntektTjeneste.finnInntekt(any(FinnInntektRequest.class))).thenReturn(opprettTestdataForInntekt());
 * Personinfo personinfo = new Personinfo.Builder()
 ...
 * RegisterdataInnhenterImpl innhenter = lagRegisterdataInnhenter(repositoryProvider);
 * final Interval interval = IntervallUtil.byggIntervall(LocalDate.now().minusMonths(3), LocalDate.now());
 * List<Inntekt> inntekter = innhenter.innhentInntektsopplysningerForSøker(personinfo, mock(Behandling.class),
 * interval);
 *
 * // Assert
 * assertThat(inntekter.size()).isEqualTo(2);
 * assertThat(inntekter.get(0).getArbeidsgiverIdentifikator()).isEqualTo(UTBETALER1);
 * assertThat(inntekter.get(0).getBeløp()).isEqualTo(UTBETALER1_MND1.add(UTBETALER1_MND2).intValue());
 * assertThat(inntekter.get(0).getFom()).isEqualTo(FOM.atDay(1));
 * assertThat(inntekter.get(0).getAktørId()).isEqualTo(1L);
 * assertThat(inntekter.get(0).getTom()).isEqualTo(FOM.plusMonths(1).atEndOfMonth());
 * assertThat(inntekter.get(1).getArbeidsgiverIdentifikator()).isEqualTo(UTBETALER2);
 * assertThat(inntekter.get(1).getBeløp()).isEqualTo(UTBETALER2_MND2.add(UTBETALER2_MND3).add(UTBETALER2_MND4).intValue(
 * ));
 * assertThat(inntekter.get(1).getFom()).isEqualTo(FOM.plusMonths(1).atDay(1));
 * assertThat(inntekter.get(1).getTom()).isEqualTo(TOM.atEndOfMonth());
 * }
 *
 * private List<Månedsinntekt> opprettTestdataForInntekt() {
 * List<Månedsinntekt> månedsinntekter = new ArrayList<>();
 *
 * månedsinntekter.add(new Månedsinntekt.Builder()
 * .medArbeidsgiver(UTBETALER1)
