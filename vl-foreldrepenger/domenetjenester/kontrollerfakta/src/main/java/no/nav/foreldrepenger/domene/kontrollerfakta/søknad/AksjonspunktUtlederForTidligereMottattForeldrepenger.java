...
@ApplicationScoped
public class AksjonspunktUtlederForTidligereMottattForeldrepenger implements AksjonspunktUtleder {
    ...
    // For CDI.
    public AksjonspunktUtlederForTidligereMottattForeldrepenger() {
    }

    @Inject
    public AksjonspunktUtlederForTidligereMottattForeldrepenger(BehandlingRepositoryProvider repositoryProvider) {
    ...
    private Utfall harMottattStønadSiste10Mnd(Behandling behandling, AktørYtelse aktørYtelse) {
        ...
        if (aktørYtelse.getYtelser().isEmpty()) {
            ...
            return (finnYtelse(aktørYtelse) == JA || harMottattStønadFraVLDeSiste10Mnd(aktørYtelse.getYtelser(), søknad) == JA)
                ? JA
                : NEI;
        }
    }
