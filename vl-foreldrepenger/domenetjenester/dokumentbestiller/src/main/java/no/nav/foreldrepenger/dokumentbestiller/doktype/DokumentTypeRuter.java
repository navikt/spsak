...
public class DokumentTypeRuter {
    ...
    public static DokumentTypeMapper dokumentTypeMapper(DokumentData dokumentData, KodeverkRepository kodeverkRepository) throws InstantiationException, IllegalAccessException {
        String kode = dokumentData.getDokumentMalType().getKode();
        if (DokumentMalType.KLAGE_AVVIST_DOK.equals(kode)) {
            return new KlageAvvistBrevMapper(kodeverkRepository);
        }
