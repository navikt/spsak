...
public class TilJournalføringTaskTest {
    ...
    public void setup() {
        ...
        TilJournalføringTjeneste tilJournalføringTjeneste = new TilJournalføringTjeneste(journalTjenesteMock, fagsakRestKlient, dokumentRepositoryMock);
    ...
    public void test_mangler_ting_som_ikke_kan_rettes() {
        ...
        journalføringsbehov.leggTilJournalMangel(JournalPostMangler.JournalMangelType.INNHOLD, true);
        when(journalTjenesteMock.utledJournalføringsbehov(ARKIV_ID)).thenReturn(journalføringsbehov);
    ...
    public void test_skalKasteTekniskExceptionNårJournalTilstandIkkeErEndelig() {
        ...
        expectedException.expectMessage("FP-785833");

        task.doTask(data);

        verify(journalTjenesteMock).journalførDokumentforsendelse(any(DokumentforsendelseRequest.class));
        verify(dokumentRepositoryMock, times(0)).oppdaterForsendelseMetadata(any(UUID.class), any(), any(), any(ForsendelseStatus.class));
    }
}
