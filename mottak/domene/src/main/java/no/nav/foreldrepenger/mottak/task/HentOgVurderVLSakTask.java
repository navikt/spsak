...
public class HentOgVurderVLSakTask extends WrappedProsessTaskHandler {
    ...
    public MottakMeldingDataWrapper doTask(MottakMeldingDataWrapper dataWrapper) {
         ...
         *
         * TODO (diamant): potensiell infinite task-loop brytes etter en dag av vurder
         */
        ...
        if (behandlendeFagsystemDto.isBehandlesIVedtaksløsningen()) {
            if (behandlendeFagsystemDto.getSaksnummer().isPresent()) {
            ...
            nesteSteg = dataWrapper.nesteSteg(OpprettGSakOppgaveTask.TASKNAME);
        } else if (behandlendeFagsystemDto.isPrøvIgjen()){
