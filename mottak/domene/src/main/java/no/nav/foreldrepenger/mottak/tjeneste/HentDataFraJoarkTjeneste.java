...
public class HentDataFraJoarkTjeneste {
    ...
    public static LocalDate hentForsendelseMottatt(List<JournalMetadata<DokumentTypeId>> hovedDokumenter) {
        return hovedDokumenter.stream().filter(m -> m.getForsendelseMottatt() != null).map(JournalMetadata::getForsendelseMottatt).findFirst().orElse(LocalDate.now(FPDateUtil.getOffset()));
    }
